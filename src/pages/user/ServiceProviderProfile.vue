    ...
    <section class="bg-cb-card rounded-md">
      <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        <div class="pt-5 pb-0 flex items-center justify-between">
          <button @click="goBack" class="inline-flex items-center gap-1.5 text-sm text-cb-muted hover:text-cb-text transition-colors">
            <i class="fa-solid fa-arrow-left text-xs"></i> Back
          </button>
          <span class="inline-flex items-center gap-1.5 rounded-full border border-cb-divider bg-cb-field px-3 py-1 text-[11px] font-semibold text-cb-muted">
            <i class="fa-solid fa-globe text-[9px]"></i> Public Profile
          </span>
        </div>
        ...
        <template v-else-if="profile">
          <div class="py-7 flex flex-col gap-5 sm:flex-row sm:items-start sm:gap-7">
            <div class="relative shrink-0 self-start">
              <div class="flex h-24 w-24 items-center justify-center overflow-hidden rounded-full bg-cb-accent text-3xl font-bold text-cb-contrast ring-4 ring-cb-card sm:h-28 sm:w-28">
                <img 
                  v-if="profile.avatar" 
            ...
            <div class="flex-1 min-w-0">
              <div class="flex flex-wrap items-center gap-2">
                <h1 class="text-2xl font-bold text-cb-text sm:text-3xl leading-tight">{{ profile.displayName }}</h1>
                <span v-if="profile.identityVerificationBadge" class="inline-flex items-center gap-1 rounded-full bg-cb-accent/10 px-2.5 py-0.5 text-xs font-semibold text-cb-accent">
                  <i class="fa-solid fa-circle-check text-[9px]"></i> Verified
                </span>
              </div>

              <div class="mt-3 flex flex-wrap gap-2">
                <span class="inline-flex items-center gap-1.5 rounded-full border border-cb-divider bg-cb-field px-3 py-1 text-xs font-medium text-cb-text">
                  <i class="fa-solid fa-user-tag text-[9px] text-cb-muted"></i>
                  {{ roleLabel }}
                </span>
                <span v-if="profile.institutionName" class="inline-flex items-center gap-1.5 rounded-full border border-cb-divider bg-cb-field px-3 py-1 text-xs font-medium text-cb-text">
                  <i class="fa-solid fa-graduation-cap text-[9px] text-cb-muted"></i>
            ...
            <div class="shrink-0 self-start">
              <button @click="copyProfileLink" class="flex items-center gap-2 rounded-xl border border-cb-divider bg-cb-field px-4 py-2.5 text-sm font-semibold text-cb-muted hover:text-cb-accent transition-all">
    ...
    <div v-if="profile" class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-6 space-y-5">
      ...
      <div class="rounded-2xl border border-cb-divider bg-cb-card p-5">
        <div class="mb-3 flex items-center justify-between">
          <p class="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-cb-muted">
          ...
          <button 
            ...
            @click="startEditBio"
            class="text-[11px] font-bold text-cb-accent hover:underline flex items-center gap-1"
          >
        ...
        <div v-if="!isEditingBio">
          <p class="text-sm leading-relaxed text-cb-text whitespace-pre-line">
        ...
        <div v-else class="space-y-4">
          <textarea
            v-model="editBioText"
            rows="5"
            class="w-full rounded-xl border border-cb-divider bg-cb-field p-4 text-sm text-cb-text focus:border-cb-accent outline-none transition-all resize-none"
            placeholder="Write your bio here..."
          ></textarea>
          <div class="flex justify-end gap-3">
            <button @click="isEditingBio = false" class="px-4 py-2 text-xs font-semibold text-cb-muted">Cancel</button>
            <button 
              @click="handleSaveBio" 
              :disabled="savingBio"
              class="flex items-center gap-2 rounded-lg bg-cb-accent px-5 py-2 text-xs font-bold text-white disabled:opacity-50"
            >
      ...
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div class="rounded-2xl border border-cb-divider bg-cb-card px-4 py-4 text-center">
          <p class="text-2xl font-bold text-cb-text">{{ profile.totalOrdersCompleted || 0 }}</p>
          <p class="text-xs text-cb-muted">Orders</p>
        </div>
        <div class="rounded-2xl border border-cb-divider bg-cb-card px-4 py-4 text-center">
          <p class="text-2xl font-bold text-cb-text">{{ profile.averageRating || '0.0' }}</p>
          <p class="text-xs text-cb-muted">Rating</p>
        </div>
        <div class="rounded-2xl border border-cb-divider bg-cb-card px-4 py-4 text-center">
          <p class="text-2xl font-bold text-cb-text">{{ profile.totalReviews || 0 }}</p>
          <p class="text-xs text-cb-muted">Reviews</p>
        </div>
        <div class="rounded-2xl border border-cb-divider bg-cb-card px-4 py-4 text-center">
          <p class="text-2xl font-bold text-cb-text truncate">{{ formatLastSeen }}</p>
          <p class="text-xs text-cb-muted">Last Active</p>
        </div>
      </div>

      <div v-if="recentOrders.length" class="rounded-2xl border border-cb-divider bg-cb-card p-5">
        <p class="mb-4 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-cb-muted">
        ...
        <div class="space-y-3">
          <div v-for="order in recentOrders" :key="order._id" class="flex items-center justify-between p-3 rounded-xl bg-cb-field border border-cb-divider">
            <div class="min-w-0">
              <p class="text-sm font-semibold text-cb-text truncate">Order #{{ order._id.slice(-6).toUpperCase() }}</p>
              <p class="text-[10px] text-cb-muted">{{ new Date(order.createdAt).toLocaleDateString() }}</p>
            </div>
            <span class="px-2 py-1 rounded-md bg-cb-accent/10 text-[10px] font-bold text-cb-accent">COMPLETED</span>
          </div>
        </div>
      </div>
      ...
      <div v-if="recentErrands.length" class="rounded-2xl border border-cb-divider bg-cb-card p-5">
        ...
        <div class="space-y-3">
          <div v-for="errand in recentErrands" :key="errand._id" class="p-3 rounded-xl bg-cb-field border border-cb-divider">
             <p class="text-sm font-semibold text-cb-text capitalize">{{ errand.title || 'Errand' }}</p>
          </div>
        </div>
      </div>
